...
void setup() {
    ...
    */
    ...
    if (isnan(offset_HX711_N1_ChannelA) || offset_HX711_N1_ChannelA == 0.0 || !tareValide) {
        ...
        Serial.println("*** ATTENTION: CALIBRATION DE LA TARE EN COURS ***");
        Serial.println("*** LA BALANCE DOIT ETRE COMPLETEMENT VIDE ! ***");
        Serial.println("*** Si il y a du poids dessus, ETEIGNEZ et ENLEVEZ le poids d'abord ! ***");
        Serial.println("");
        Serial.println("Attente de 5 secondes pour vous permettre de verifier...");
        delay(5000);  // Délai de 5 secondes pour laisser le temps de réagir
